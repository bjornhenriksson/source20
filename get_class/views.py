...
from get_class.models import Course
...
def index(request):
    ...
    get_url_list = Course.objects.filter(course_name=current_course, id=page)
    get_previous_id = int(page) - 1
    get_next_id = int(page) + 1
    if Course.objects.filter(id=get_next_id).exists():
        next_page = '<a href="?course=%s&page=%s">next</a>' % (current_course,get_next_id)
    else:
        next_page = 'no more pages'
    if Course.objects.filter(id=get_previous_id).exists():
